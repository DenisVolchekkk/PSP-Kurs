...
class WebSocketServer
{
    ...
    static async Task Main()
    {
        ...
        while (true)
        {
            try
            {
                ...
                imageService = new ImageService(context, fileManager, registeredUsers);
    ...
    static async Task HandleHttpRequest(HttpListenerContext context)
    {
        try
        {
            ...
            {
                await galleriesService.Delete();
            }
            else if (method == "POST" && path == "/editgallery") // Добавлено для редактирования галереи
            {
